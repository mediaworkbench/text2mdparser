...
def test_call_lm_studio_api_success(mock_requests_post, caplog):
    ...
    result = call_lm_studio_api(SAMPLE_TEXT_CONTENT, SAMPLE_API_URL)
    ...
    assert f"Calling LM Studio API at {SAMPLE_API_URL}..." in caplog.text
    assert f"API call to {SAMPLE_API_URL} successful, content received." in caplog.text

def test_call_lm_studio_api_with_model_identifier(mock_requests_post):
    ...
    call_lm_studio_api(SAMPLE_TEXT_CONTENT, SAMPLE_API_URL, model_identifier=test_model_id)
...
def test_call_lm_studio_api_with_empty_model_identifier(mock_requests_post):
    ...
    call_lm_studio_api(SAMPLE_TEXT_CONTENT, SAMPLE_API_URL, model_identifier="")
...
def test_call_lm_studio_api_with_custom_system_prompt(mock_requests_post):
    ...
    call_lm_studio_api(SAMPLE_TEXT_CONTENT, SAMPLE_API_URL, system_prompt=custom_prompt)
...
def test_call_lm_studio_api_with_custom_temperature(mock_requests_post):
    ...
    call_lm_studio_api(SAMPLE_TEXT_CONTENT, SAMPLE_API_URL, temperature=0.3)
...
def test_call_lm_studio_api_with_max_tokens(mock_requests_post):
    ...
    call_lm_studio_api(SAMPLE_TEXT_CONTENT, SAMPLE_API_URL, max_tokens=2000)
...
def test_call_lm_studio_api_without_max_tokens(mock_requests_post):
    ...
    call_lm_studio_api(SAMPLE_TEXT_CONTENT, SAMPLE_API_URL, max_tokens=None)
...
def test_call_lm_studio_api_success_with_api_key(mock_requests_post):
    ...
    call_lm_studio_api(SAMPLE_TEXT_CONTENT, SAMPLE_API_URL, api_key=api_key)
...
def test_call_lm_studio_api_http_error(mock_requests_post, caplog):
    ...
    assert f"API request to {SAMPLE_API_URL} failed with status code 500. Response: Internal Server Error" in caplog.text
...
def test_call_lm_studio_api_request_exception_timeout(mock_requests_post, caplog):
    ...
    assert result is None
    assert f"API request to {SAMPLE_API_URL} timed out after 60 seconds." in caplog.text
...
def test_call_lm_studio_api_connection_error(mock_requests_post, caplog):
    ...
    assert result is None
    assert f"Cannot connect to LM Studio API at {SAMPLE_API_URL}" in caplog.text
...
def test_call_lm_studio_api_request_exception_generic(mock_requests_post, caplog):
    ...
    assert result is None
    assert f"API request to {SAMPLE_API_URL} failed due to a network or request issue: Generic network error" in caplog.text
...
def test_call_lm_studio_api_json_decode_error(mock_requests_post, caplog):
    ...
    assert result is None
    assert f"Could not decode JSON response from {SAMPLE_API_URL}. Response text: This is not valid JSON" in caplog.text
...
def test_call_lm_studio_api_unexpected_response_structure(mock_requests_post, caplog):
    ...
    assert result is None
    assert f"Unexpected response structure from {SAMPLE_API_URL}. Full response: {json.dumps({'not_choices': []})}" in caplog.text
...
def test_call_lm_studio_api_unexpected_exception(mock_requests_post, caplog):
    ...
    assert result is None
    assert f"An unexpected error occurred during the API call to {SAMPLE_API_URL}: A totally unexpected error!" in caplog.text
